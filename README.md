...
- [x] Scoring-reference RAG — CEFR band descriptors embedded through the gateway's `/v1/embeddings` (OpenAI) into pgvector (`rubric_chunks`), cosine-ordered and `exam_section`-filtered, injected into the score node to anchor writing + speaking grading. **kNN was measured, then deliberately not relied on:** English descriptors against French productions over a 6-band corpus returned the same low bands for every essay, so top-*k* grounding was uniform and uninformative. `k` therefore defaults to the full per-section ladder — the score node places the production against the whole scale, and cosine distance is kept only as a nearest-first ordering hint. Retrieval starts selecting again once the corpus is big enough to have something to leave out (exemplars, per-dimension chunks). Additive either way: grading falls back to the pre-RAG path when the store is empty or embeddings are unavailable
